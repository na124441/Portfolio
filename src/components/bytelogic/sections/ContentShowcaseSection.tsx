...
export const ContentShowcaseSection: React.FC = () => {
  ...
  return (
    <section id="content" className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 border-t border-[#1C2830]">
      {/* Header */}
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div>
          ...
          <p className="mt-2 text-sm text-[#A8B3BA] font-sans max-w-xl">
        ...
        {/* Filter Pills */}
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 bl-scrollbar text-xs font-mono">
          {filterTabs.map((tab) => (
            <button
              key={tab.key}
              onClick={() => setSelectedFormat(tab.key)}
              className={cn(
                'px-3 py-1.5 rounded-[4px] border text-[11px] whitespace-nowrap transition-colors cursor-pointer',
